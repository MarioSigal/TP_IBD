...
\author{Teo Nabot \and Mario Sigal \and Felipe Pasquet (LU: 1084/22)}
\date{}
...
\begin{figure}[ht]
    ...
    \caption*{\textbf{FIGURA 1:} \textit{Diagramas Entidad-Relación (antiguo vs nuevo),
    mostrando las decisiones de diseño tomadas} (Imágenes con mayor nitidez disponibles
    en el anexo).}
\end{figure}
...
cambios respecto al modelo original del negocio se pueden ver en la FIGURA 1.
...
\begin{itemize}[nosep]
    ...
    \texttt{MARCAS.nombre}, \texttt{PRODUCTOS.sku}, \texttt{PROVEEDORES.cuit},
    \texttt{PUNTOS\_DE\_VENTA.nombre}, \texttt{CLIENTES.dni}, \texttt{CLIENTES.email}.
\end{itemize}
...
Como \texttt{VENTAS} ya no almacena columnas numéricas, las tres consultas parten de una
CTE (\texttt{WITH}) \texttt{ventas\_ext} que reconstruye
\texttt{precio\_total}/\texttt{costo\_total} por venta desde el detalle. Esto cumple
además el requisito de usar Common Table Expressions.
...
\end{figure}
\captura{\textbf{CAPTURA 9:} \textit{Resultado de C1 (perfil de las columnas;
\texttt{cliente\_id} con \% no nulos < 100).}}
...
\end{figure}
\captura{\textbf{CAPTURA 10:} \textit{Resultado de C2 (estadísticos de
\texttt{precio\_total} y \texttt{costo\_total}).}}
...
\end{figure}
\captura{\textbf{CAPTURA 11:} \textit{Resultado de C3 para \texttt{metodo\_pago} y
\texttt{puntos\_de\_venta\_id}.}}
...
\subsubsection*{Plan de ejecución (EXPLAIN ANALYZE)}
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\linewidth]{analyze1.png}
\end{figure}
Ejecutamos \texttt{EXPLAIN ANALYZE} sobre la consulta 1 y obtuvimos la siguiente
estructura de plan (de abajo hacia arriba), junto al costo que aporta cada operación:
...
\midrule
Seq Scan sobre \texttt{detalle\_ventas}                  & 262   & — \\
Sort por \texttt{product\_id} (entrada del Merge Join)   & 1\,143 & +880 \\
Merge Join + Nested Loop con productos/categorías        & 1\,359 & +216 \\
Incremental Sort + GroupAggregate (el \texttt{GROUP BY}) & 2\,366 & +1\,007 \\
Sort por (\texttt{categoria\_id}, \texttt{profit\_total} DESC) & 5\,585 & +3\,122 \\
WindowAgg (calcula \texttt{DENSE\_RANK})                  & 5\,843 & +258 \\
Sort final por (\texttt{categoria}, \texttt{posicion\_en\_categoria}) & \textbf{9\,094} & +3\,122 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Notamos que} los \textbf{tres nodos Sort} concentran casi todo el costo. El Seq
Scan sobre \texttt{detalle\_ventas} (262) es barato y óptimo: como la consulta agrega toda
la tabla \textbf{sin WHERE}, leerla entera de forma secuencial es lo esperable. Notamos
que aparecían \textbf{Index Scan sin haber creado índices}: son \texttt{productos\_pkey} y
\texttt{categorias\_pkey}, los índices \textbf{implícitos} que respaldan las PK. El join se
hace por PK (\texttt{c.categoria\_id = p.categoria\_id}) sobre tablas chicas, y buscar «la
fila cuya PK = X» por índice es óptimo, por eso el planner los elige (con Memoize cacheando
categorías ya buscadas).

De los tres Sort, dos \textbf{no son indexables} porque ordenan \textbf{valores derivados}:
\texttt{profit\_total} (un SUM del \texttt{GROUP BY}) y \texttt{posicion\_en\_categoria}
(resultado del \texttt{DENSE\_RANK}). Ningún índice puede entregar pre-ordenado algo que no
existe como columna base. El \textbf{único atacable} es el Sort por \texttt{product\_id}
(+880), que ordena una columna real solo para alimentar el Merge Join. Índice propuesto:
...
\textbf{De esta manera, eliminamos el Sort por \texttt{product\_id}:} el índice entrega las
filas ya ordenadas por \texttt{product\_id}, justo lo que el Merge Join necesita.
...
\end{figure}
\captura{\textbf{SOLO RESULTADO DE CAMBIO EN EL SORT CON ÍNDICE (SI NO CAMBIA NADA MÁS)
+ MINI CONCLUSIÓN}}
...
\end{figure}
\captura{\textbf{CAPTURA 15:} \textit{Tabla por sucursal: facturación total, cantidad de
...
es similar a la segunda consulta de la etapa 2. Podemos ver que esta manera de resolverla
es bastante más sencilla e intuitiva.
...
\end{figure}
\captura{\textbf{CAPTURA 16:} \textit{Top 10 \texttt{product\_id} por ganancia total
acumulado.}}
...
\end{figure}
\captura{\textbf{CAPTURA 17:} \textit{Distribución por método de pago (cantidad, \% y
monto).}}
